...
import { FlatList, Pressable, ScrollView, StyleSheet, Text, ToastAndroid, TouchableOpacity, View } from 'react-native';
...
const OrderSet = ({ navigation, route }) => {
  ...
  const [errNurseSex, setErrNurseSex] = useState('')
  ...
  const onCheckout = () => {
    ...
    formData.append('OrderNurseCriteria[sex]', nurseSex)
    formData.append('OrderNurseCriteria[experience_min]', exMax)
    formData.append('OrderNurseCriteria[experience_max]', exMax)
    ...
    getData('token').then((resToken) => {
      Axios.post(endpoint.order, formData, {
        ...
        if (status == 401) {
          ...
          showMessage(`gagal ${err?.response}`);
        }
      });
    });
  };
...
const styles = StyleSheet.create({
  ...
  button: { paddingHorizontal: 24, paddingTop: 24, position: 'absolute', bottom: 0, width: '100%', marginBottom : 20 },
  round:
