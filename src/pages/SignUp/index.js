...
const SignUp = ({ navigation }) => {
  ...
  const onSubmit = () => {
    ...
    if (form.email != '' && reg.test(form.email) === true && form.name != '' && form.phone != '' && form.password != '' && form.password == form.confirmation_password) {
    ...
    Axios.post(endpoint.registration, form)
      .then((res) => {
        ...
        if (res.data.data.otpRequired) {
          ...
          navigation.navigate('EditProfile', { profile: profile, 'register' : true })
        }
      })
      .catch((err) => {
        dispatch(setLoading(false));
