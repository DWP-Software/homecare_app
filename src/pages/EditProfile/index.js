...
const EditProfile = ({ navigation, route }) => {
  ...
  const onSubmit = () => {
    ...
    if (name.length > 0 && reg.test(email) === true && email.length > 0 && sex != "" && province.id != null && district.id != null && subdistrict.id != null && village.id != null && address != "") {
      setErrName('')
      setErrEmail('')
      setErrAge('')
      setErrSex('')
  ...
  const getProvince = () => {
    ...
    Axios.get(endpoint.province)
      .then((res) => {
        ...
        console.log("prov", res.data.data.provinces)
        setLoading(false)
      })
      .catch((err) => {
          ...
          {!route.params.register &&
            <>
              <TextInput
                ...
                value={email}
                errorMsg={errEmail}
                onChangeText={(value) => { setEmail(value), setErrEmail('') }}
              />
              <Gap height={16} />
            </>
          }
